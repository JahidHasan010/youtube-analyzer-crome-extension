...
// ===== Convert any timestamp (number, date, or YouTube-style) to Date =====
function toDateFromAny(ts) {
  try {
    if (!ts) return null;

    // If numeric — could be seconds, ms, or epoch
    if (typeof ts === 'number') {
      const val = ts < 1e12 ? ts * 1000 : ts;
      return new Date(val);
    }

    // If it's a string containing ":" → treat as YouTube video time (hh:mm:ss)
    if (typeof ts === 'string' && ts.includes(":")) {
      ...
      // Convert seconds to Date (relative timeline)
...
// ===== Format time nicely (00:00 → 1:05:30 etc.) =====
const formatTime = (ts) => {
  const date = toDateFromAny(ts);
  if (!date || isNaN(date)) return "00:00";
...
// ===== Process comments into timeline bins (every 30 seconds) =====
const processTimelineData = () => {
  try {
    const map = {};
    ...
    commentsData.forEach(c => {
      ...
      const sec = Math.floor(ts.getTime() / 1000);
      const bin = Math.floor(sec / 30) * 30; // group in 30-sec intervals
    ...
    const result = Object.values(map).sort((a, b) => a.bin - b.bin);
    logDebug("Processed YouTube timeline data:", result);
    return result;
  } catch (err) {
    logError("Error processing timeline data:", err);
    return [];
...
//     logInfo(`Sentiment: ${selectedSentiment} | Strong: ${allStrong.length} | Weak: ${allWeak.length} | Strong%: ${strongPercent}% | Weak%: ${weakPercent}%`);

//     const strongTop5 = allStrong.slice(0, 5);
//     const weakTop5 = allWeak.slice(0, 5);
//     logDebug("Top 5 strong comments:", strongTop5);
//     logDebug("Top 5 weak comments:", weakTop5);
...
//         </div>
//         <div class="breakdown-comment-list">
//           <div>
//             <h4>Top Strong 🌟</h4>
...
//     if (sentimentStrengthChart) {
//       sentimentStrengthChart.destroy();
//       logDebug("Destroyed previous sentimentStrengthChart instance.");
//     }
...
//     logInfo("✅ renderDetailedBreakdown() completed successfully with chart.");
//   } catch (err) {
...
//         // Re-render main dashboard view
//         renderDashboard();

//         logInfo("Detailed breakdown closed successfully.");
//       });
//     } else {
//       logWarn("Close breakdown button not found after rendering.");
//     }
...
//   logInfo("Starting render for Overall Sentiment Chart...");
...
//     if (!ctx) {
//       logInfo("Canvas context not found for 'overall-sentiment-chart'. Skipping render.");
//       return;
//     }

//     if (overallSentimentChart) {
//       logInfo("Destroying existing Overall Sentiment Chart instance...");
...
//     logInfo("✅ Overall Sentiment Chart rendered successfully.");
//   } catch (error) {
//     logError("Failed to render Overall Sentiment Chart.", error);
//   }
// };
...
//   logInfo("Starting render for Timeline Chart...");
...
//           {
//             label: "Positive",
//             data: data.map(d => ({ x: d.bin, y: d.positive })),
//             borderColor: "#4ade80",
...
//           {
//             label: "Neutral",
//             data: data.map(d => ({ x: d.bin, y: d.neutral })),
//             borderColor: "#facc15",
...
//           {
//             label: "Negative",
//             data: data.map(d => ({ x: d.bin, y: d.negative })),
//             borderColor: "#f87171",
...
//           x: {
//             type: "linear",
//             title: { display: true, text: "Video Time" },
...
//     logInfo("✅ Timeline Chart rendered successfully.");
//   } catch (error) {
//     logError("Failed to render Timeline Chart.", error);
//   }
// };
...
// ===== Render Polished Timeline Sentiment Chart =====
// ===== Render Timeline Sentiment (Layered Line Chart with Gradient Fill) =====
const renderTimelineChart = () => {
  logInfo("Starting polished render for Timeline Chart...");
  ...
  try {
    ...
    const ctx = safeGetContext("timeline-chart");
    if (!ctx) {
      logWarn("Canvas context not found for 'timeline-chart'.");
      return;
    }
    ...
    // Prepare data arrays
    const labels = data.map(d => formatTime(d.bin * 1000));
    const positives = data.map(d => d.positive || 0);
    const neutrals = data.map(d => d.neutral || 0);
    const negatives = data.map(d => d.negative || 0);

    if (labels.length === 0) {
      for (let i = 0; i < 10; i++) {
        labels.push(formatTime(i * 3600 * 1000));
        positives.push(Math.floor(Math.random() * 60));
        neutrals.push(Math.floor(Math.random() * 40));
        negatives.push(Math.floor(Math.random() * 20));
      }
    }

    // === Gradient Fills ===
    const gradPositive = ctx.createLinearGradient(0, 0, 0, 400);
    gradPositive.addColorStop(0, "rgba(34,197,94,0.5)");
    gradPositive.addColorStop(1, "rgba(34,197,94,0.1)");

    const gradNeutral = ctx.createLinearGradient(0, 0, 0, 400);
    gradNeutral.addColorStop(0, "rgba(250,204,21,0.5)");
    gradNeutral.addColorStop(1, "rgba(250,204,21,0.1)");

    const gradNegative = ctx.createLinearGradient(0, 0, 0, 400);
    gradNegative.addColorStop(0, "rgba(248,113,113,0.5)");
    gradNegative.addColorStop(1, "rgba(248,113,113,0.1)");
    ...
    logInfo("✅ Timeline Chart rendered successfully (gradient filled lines).");
  } catch (err) {
    logError("❌ Failed to render Timeline Chart:", err);
  }
};
